...
export function Component() {
  ...
  const throttledIncreasePage = throttle(() => {
    if (rankCardList.length < MAX_POSTS) {
  ...
  const rankData = useQuery({
      ...
      axios
        ...
        .then((res) => {
          ...
          setRankCardList((prevList) => {
            ...
            return updatedList.slice(0, MAX_POSTS);
          });
          const extractedUserIds = newItems.map((item) => item.userId);
          setUserIds((prevIds) => [
            ...new Set([...prevIds, ...extractedUserIds]),
          ]);
          setHasMore(
            rankCardList.length + newItems.length <
              Math.min(res.data.totalItems, MAX_POSTS)
          );
    ...
    enabled: rankCardList.length < MAX_POSTS,
  });
  ...
  return (
    <>
      ...
      <section className={S.container}>
        ...
        {hasMore && rankCardList.length < MAX_POSTS && (
